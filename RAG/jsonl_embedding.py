...
def chunk_text_by_segment(sentences, predicted_labels, tokenizer, title=None, doc_id=None):
    ...
    for i, is_segment_end in enumerate(predicted_labels):
        if i >= len(sentences):
            break
...
def create_segmendtaion_faiss_index_from_jsonl(jsonl_file_path, output_faiss_path, output_ids_path):
    ...
    with open(jsonl_file_path, 'r', encoding='utf-8') as jsonl_file:
        for line in jsonl_file:
            ...
            if args.chunk_type in ['256', '512', '1024', '2048']:
                ...
                text_chunks = chunk_text_by_segment(sentences, predicted_labels, tokenizer, title, doc_id)
    ...
    if args.is_cluster:
        ...
        loaded_data, embeddings, total_chunk_size, total_chunks_count = cluster_segment(
            loaded_data, embeddings, args.max_file_size, args.k, args.is_mul_vector
        )
        with open(output_ids_path, 'w', encoding='utf-8') as id_file:
    ...
    logging.info(f"Average chunk size is: {total_chunk_size/total_chunks_count}")
    print(f"Total number of chunks for chunk type {args.max_file_size} the dataset {args.dataset} is: {len(embeddings)}")
    print(f"Average chunk size is: {total_chunk_size/total_chunks_count}")
...
def main(args):
    if args.dataset:
        ...
        create_segmendtaion_faiss_index_from_jsonl(
            ...
            output_faiss_path=f'data/{args.dataset}.index',
            output_ids_path=f'data/{args.dataset}.json'
        )
    else:
        raise ValueError(f"Invalid dataset: {args.dataset}. Please choose 'squad', 'narrativeqa', or 'quality'.")
    
if __name__ == '__main__':
    parser = ArgumentParser()
    parser.add_argument('--dataset', help='Dataset name (e.g., squad, narrativeqa, quality)', required=True, type=str)
    parser.add_argument('--chunk_type', help='Chunking strategy: 256, 512, 1024, 2048, seg, semantic', type=str, default='256')
    parser.add_argument('--max_file_size', help='Maximum file size for clustering', type=int, default=0)
    parser.add_argument('--k', help='Clustering parameter k', type=float, default=0.5)
    ...
    parser.add_argument('--original_data', help='Path to data directory', type=str, default='data_512_1024')
