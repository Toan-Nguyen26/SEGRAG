...
def collate_fn_2_head(batch):
    batched_data = []
    batched_segment_targets = []
    batched_document_targets = []
    paths = []
    ...
    for data, segment_targets, document_targets, path in batch:
        try:
            max_index = len(data)
            tensored_data = []
            for curr_sentence_index in range(0, len(data)):
            ...
            # Assuming segment_targets and document_targets are lists of indices
            tensored_segment_targets = torch.zeros(len(data)).long()
            tensored_segment_targets[torch.LongTensor(segment_targets)] = 1
            tensored_segment_targets = tensored_segment_targets[:-1]

            tensored_document_targets = torch.zeros(len(data)).long()
            tensored_document_targets[torch.LongTensor(document_targets)] = 1
            tensored_document_targets = tensored_document_targets[:-1]

            batched_data.append(tensored_data)
            batched_segment_targets.append(tensored_segment_targets)
            batched_document_targets.append(tensored_document_targets)
            paths.append(path)
        except Exception as e:
            logger.info('Exception "%s" in file: "%s"', e, path)
            logger.debug('Exception!', exc_info=True)
    ...
    return batched_data, batched_segment_targets, batched_document_targets, paths
